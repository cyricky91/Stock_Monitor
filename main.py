...
def get_ai_comment(ticker, price, rsi, poc, action, advice):
    ...
    【關鍵數據】
    - 現價：{price}
    - 籌碼重心 (POC)：{poc} (此為核心成本區)
    - RSI：{rsi}
    - 主力行為：{action}

    【分析任務】
    請依照以下格式，提供不超過 150 字的繁體中文分析：
    1. **趨勢定調**：判斷目前是「多頭回調」、「空頭壓制」還是「高位派發」。
    2. **具體買入建議**：給出一個價格區間（例如：POC ± 2% 範圍）。
    3. **分批止盈目標**：參考近期高點或布林帶上軌給出第一目標價。
    4. **防守位 (止損)**：給出精確價格（建議設在 POC 下方 3% 或近期支撐位）。
    5. **警示**：若 RSI 超過 70，提醒追高風險。
