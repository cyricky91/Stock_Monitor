...
def get_ai_comment(ticker, price, rsi, poc, action, advice):
    ...
    prompt = f"""
    你是一位專注於港股/美股的『籌碼專家』。請針對 {ticker} 進行多維度分析：

    數據狀態：
    - 當前價格: {price:.2f} (相對 POC {poc:.2f} 的位置)
    - 強弱指標: RSI {rsi:.1f}
    - 主力動作: {action}

    請嚴格按以下結構回覆（100字內）：
    1.【形勢分析】：簡述目前是『多頭強勢回測』還是『空頭弱勢反彈』。
    2.【關鍵位】：具體給出『買入區間』（例：${poc*0.98:.2f}-${poc:.2f}）與『第一止盈目標』。
    3.【止損策略】：給出一個『不可挑戰的止損價』。
    4.【風險提醒】：針對 RSI 或成交量給出一句警示。
...
async def analyze_stock(ticker):
    ...
    try:
        ...
        # 獲取 AI 點評
        ai_note = get_ai_comment(ticker, latest_p, cur_rsi, poc_price, main_action, advice)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ticker in tickers:
        temp_msg = await update.message.reply_text(f"🔍 正在獲取數據並請求 DeepSeek AI 分析 {ticker}...")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 您好！傳送股票代號（如 700 或 NVDA），我將結合數據與 AI 為您診斷。")
